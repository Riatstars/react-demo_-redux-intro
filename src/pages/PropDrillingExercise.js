...
const RootComponent = (props) => {
  ...
  const addProductToCart = (sentProduct) => {
    setCart({
      products: cart.products.map((product) => {
        if (product.id !== sentProduct.id) return product;
        return {
          ...product,
          qty: (product.qty += 1),
          price: (product.price += sentProduct.price),
        };
      }),
      totalPrice: (cart.totalPrice += sentProduct.price),
    });
  };
  ...
  const removeProductFromCart = (sentProduct) => {
    setCart({
      products: cart.products.map((product) => {
        if (product.id !== sentProduct.id) return product;
